...
function Assert-VscodeExtensionContract {
    ...
    if (-not $ExtensionSource.Contains('const WORKFLOW_OPTION_LABELS = catalogItemLabels(editorMetadata.syntaxCatalog.workflow_options)') -or -not $ExtensionSource.Contains('workflowOptionLabels: WORKFLOW_OPTION_LABELS') -or -not $CodeActionProviderSource.Contains('this.workflowOptionLabels = Array.isArray(options.workflowOptionLabels)') -or -not $CodeActionProviderSource.Contains('workflowOptionLabels: this.workflowOptionLabels') -or -not $LocalCodeActionsSource.Contains('withOptionAliasAction(document, diagnostic, options.workflowOptionLabels)') -or -not $LocalCodeActionsSource.Contains('workflowOptionLabelSet(workflowOptionLabels)')) {
        throw "VS Code with-option alias quick fixes must use generated workflow option catalog labels"
    }
