...
function Assert-VscodeExtensionContract {
    ...
    foreach ($ForbiddenVscodeReadmeWording in @("promotion skeletons", "migration skeletons", "skeleton repairs", "fallback repairs", "fallback state")) {
        if ($VscodeReadmeSource.Contains($ForbiddenVscodeReadmeWording)) {
    ...
    foreach ($RequiredDiagnosticsSourceColumnToken in @(
        ...
        "uncertaintyArgumentDiagnosticRange(lineText, item)",
        "namedArgumentValueRange(lineText, [`"samples`", `"n`"])",
        "namedArgumentValueRange(lineText, [`"std`", `"sigma`", `"uncertainty`"])",
        "namedArgumentValueRange(lineText, [`"scale`", `"gain`"])",
    ...
    foreach ($RequiredSemanticDebugToken in @(
        ...
        "function themeFallbackScopeStatus",
        ...
        "tokensMissingThemeFallbackScope = tokenRows.filter",
        ...
        "semanticTokenTextCounts(tokenRows, (line) =>",
        "function addSemanticTokenPhraseCounts(counts, tokenRows, lineTextForRow)",
        "sourceLine.slice(currentEnd, next.start)",
        "if (!/^\s+$/.test(gap)) break",
        ...
        'label: "Public fields"',
        'filter: "property"',
        "arrayOrEmpty(syntaxCatalog.public_types)",
        "catalogItemLabels(syntaxCatalog.quantities)",
...
function Invoke-IdeCheck {
    ...
    foreach ($RequiredIdeToken in @(
        ...
        "copyVisibleProblems",
        "copyCursorProblemBtn",
        "copyCursorProblem",
        "problemAtCursor",
        "problemCaretDistance",
        "problemSeverityRank",
        "Copy at cursor",
        "current or nearest same-line diagnostic",
        "problemCopyText",
        ...
        "documentSymbolBreadcrumbPath",
        "documentSymbolOwnsScope",
        "renderEditorBreadcrumbs",
        "navigateEditorBreadcrumb",
        "updateEditorBreadcrumbs",
        "editorBreadcrumbs",
        "normalizedTabEditorView",
        "currentEditorViewSnapshot",
        ...
        "editorBracketOverlayContent",
        "editorBracketIndex",
        "editorBracketDecorations",
        "renderEditorBracketDecorations",
        "updateEditorBracketOverlay",
        "syncEditorBracketOverlayScroll",
        "bracketIndex.activeOffsets.has(candidateOffset)",
        ...
        "workspaceRunSaveRequests",
        "workspaceRunBuffersAreSaved",
        "workspaceRunBufferSourcesAreCurrent",
        "persistTabSaveRequests",
        ...
        "semanticTokenRange(sourceLine, token)",
        "sourceLine.slice(currentEnd, next.start)",
        "if (!/^\s+$/.test(gap)) break",
        'key: "type"',
        'key: "quantity"',
        ...
        "semanticTokenText",
        "highlightFilterChip",
        "highlightChipRoleClass",
        "semanticTokenTypeClass",
        "semanticTokenModifierClass",
        "semanticTokenSelectorClass",
        '`token-${safeKind}`',
    ...
    foreach ($RequiredNativeIdeSafeSaveToken in @(
        ...
        "native_ide_save_does_not_rewrite_matching_source",
...
function Invoke-LspCheck {
    ...
    foreach ($RequiredPersistentLspToken in @(
        ...
        "strict_utf16_character_to_byte",
...
function New-UserGuidePdf {
    ...
    $sections = @(
        ...
        @{ Kind = "step"; Text = "Diagnostics and role-aware colors refresh from the unsaved buffer after a short typing pause. Use Check to force an immediate refresh; error and warning counts are visible in the toolbar and details are listed in Problems." },
