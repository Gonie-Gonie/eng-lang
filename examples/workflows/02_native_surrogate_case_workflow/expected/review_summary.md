...
native `apply run_case over case_inputs` sequential scheduler rows calculating annual_electricity, annual_cooling, peak_cooling, and unmet_hours per case with zero external processes
per-case `result.json` and `case_run_manifest.json` artifacts with expression output metadata, hashes, runner, scheduler, and success status
