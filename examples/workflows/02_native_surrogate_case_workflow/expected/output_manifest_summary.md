...
outputs/prediction_designs_standard.txt
...
database plus DB write manifest should be classified as `db_write`. The generated sample table files should be classified as `standard_file` artifacts written from native sample tables. Native
...
by the workflow, while the standard-text sample table files expose the full generated row values. `process_results.json` should show
`process_count = 0`.
