...
pub use behavior::{
    component_behavior_calls, ComponentBehaviorCall, BEHAVIOR_CONTRACT_RESOLVED,
    BEHAVIOR_GRAPH_EXECUTED, BEHAVIOR_GRAPH_NOT_EXECUTED, BEHAVIOR_IDENTITY_CONTRACT,
    BEHAVIOR_IDENTITY_RUNTIME, BEHAVIOR_JACOBIAN_POLICY, BEHAVIOR_PROFILE_POLICY,
    BEHAVIOR_RELATIONSHIP_EXECUTED, BEHAVIOR_RELATIONSHIP_RESOLVED,
    BEHAVIOR_RUNTIME_DIAGNOSTICS_AVAILABLE, BEHAVIOR_RUNTIME_DIAGNOSTICS_UNAVAILABLE,
...
///
/// This compatibility entry point retains the fast-binding-only contract. New integrations that
/// also accept explicit scalar declarations should use
...
pub fn recheck_scalar_binding_suffix_incrementally(
    report: &CheckReport,
    previous_source: &str,
    source: &str,
) -> Option<CheckReport> {
...
///
/// This compatibility entry point retains the explicit-declaration-only contract. New integrations
...
fn recheck_scalar_declaration_suffix_incrementally_with_mode(
    ...
    reused.syntax_summary.fast_bindings = reused
        ...
        .fast_bindings
        .checked_sub(previous_suffix.fast_binding_count)?
        .checked_add(source_suffix.fast_binding_count)?;
...
#[derive(Clone, Debug, Eq, PartialEq)]
struct ScalarDeclarationRecord {
    name: String,
    line: usize,
...
struct ParsedScalarDeclarationSuffix {
...
fn parse_scalar_declaration_suffix(
    lines: &[source::SourceLine],
) -> Option<ParsedScalarDeclarationSuffix> {
    let program = parse_incremental_top_level_lines(lines)?;
    let syntax_summary = program.summary();
    let declarations = program
        .items
        .iter()
        .map(|item| match item {
            AstItem::FastBinding(binding) => Some(ScalarDeclarationRecord {
                name: binding.name.clone(),
                line: binding.line,
                kind: ScalarDeclarationKind::FastBinding,
            }),
            AstItem::ExplicitDecl(declaration) => Some(ScalarDeclarationRecord {
    ...
    let fast_binding_count = declarations
        .iter()
    ...
    Some(ParsedScalarDeclarationSuffix {
        program,
        declarations,
        fast_binding_count,
        explicit_declaration_count,
...
fn parse_scalar_declaration_prefix(
    lines: &[source::SourceLine],
) -> Option<ParsedScalarDeclarationPrefix> {
    let program = parse_incremental_top_level_lines(lines)?;
    let syntax_summary = program.summary();
    ...
    for item in &program.items {
        match item {
            AstItem::FastBinding(binding) if binding.context == ParseContext::TopLevel => {
                fast_binding_count = fast_binding_count.checked_add(1)?;
            }
            ...
            AstItem::Import(import) if incremental_import_is_supported(import) => {
    ...
    let declaration_count = fast_binding_count
        .checked_add(explicit_declaration_count)?
        .checked_add(const_declaration_count)?;
    if declaration_count.checked_add(imports.len())? != syntax_summary.ast_items
        || syntax_summary.imports != imports.len()
        || syntax_summary.fast_bindings != fast_binding_count
        || syntax_summary.explicit_declarations != explicit_declaration_count
        || syntax_summary.const_declarations != const_declaration_count
    {
    ...
    Some(ParsedScalarDeclarationPrefix {
        imports,
        fast_binding_count,
        explicit_declaration_count,
        const_declaration_count,
...
fn parse_incremental_top_level_lines(lines: &[source::SourceLine]) -> Option<ParsedProgram> {
...
fn incremental_import_is_supported(import: &ImportDecl) -> bool {
    if import.kind == "file" {
        return !import.target.trim().is_empty();
    }
    if !matches!(import.kind.as_str(), "use" | "import")
        || !stdlib_module_import_target_is_valid(import.target.trim())
    {
        return false;
    }

    bundled_module_registry().is_ok_and(|registry| {
        registry.modules.iter().any(|module| {
            module.name == import.target
                && matches!(
...
fn incremental_scalar_report_shape(
...
) -> Option<IncrementalScalarReportShape> {
    let imported_const_count = report
        .semantic_program
        .consts
        .iter()
    ...
    let source_ids = report
        ...
        .collect::<HashSet<_>>();
    let imported_source_ids = report
        .source_files
        .iter()
        .skip(1)
        .map(|source| source.source_id)
        .collect::<HashSet<_>>();
    let file_import_count = report
        .semantic_program
        .imports
        .iter()
        .filter(|import| import.kind == "file")
        .count();
    if root_source.source_id != SourceSpan::ROOT_SOURCE_ID
        || source_ids.len() != report.source_files.len()
        || imported_source_ids.contains(&SourceSpan::ROOT_SOURCE_ID)
    ...
    if !report.semantic_program.consts[..imported_const_count]
        .iter()
        .all(|constant| {
            imported_source_ids.contains(&constant.span.source_id)
                && constant.importable
                && constant.type_span.source_id == constant.span.source_id
                && constant.expression_span.source_id == constant.span.source_id
                && constant
                    .unit_span
                    .is_none_or(|span| span.source_id == constant.span.source_id)
        })
        || !report.semantic_program.consts[imported_const_count..]
            .iter()
            .all(|constant| {
                constant.span.source_id == SourceSpan::ROOT_SOURCE_ID
                    && constant.type_span.source_id == SourceSpan::ROOT_SOURCE_ID
                    && constant.expression_span.source_id == SourceSpan::ROOT_SOURCE_ID
                    && constant
                        .unit_span
                        .is_none_or(|span| span.source_id == SourceSpan::ROOT_SOURCE_ID)
            })
        || !report.semantic_program.expected_types[..imported_const_count]
            .iter()
            .all(|expected| imported_source_ids.contains(&expected.span.source_id))
        || !report.semantic_program.expected_types[imported_const_count..]
            .iter()
            .all(|expected| expected.span.source_id == SourceSpan::ROOT_SOURCE_ID)
        || !report.semantic_program.typed_bindings[..imported_const_count]
            .iter()
            .all(|binding| imported_source_ids.contains(&binding.span.source_id))
        || !report.semantic_program.typed_bindings[imported_const_count..]
            .iter()
            .all(|binding| binding.span.source_id == SourceSpan::ROOT_SOURCE_ID)
        || !report
            .inferred_declarations
            .iter()
    ...
    let inferred_keys = report
        ...
        .map(|declaration| {
            (
                ...
                declaration.expression_span.source_id,
                declaration.line,
            )
        })
        .collect::<HashSet<_>>();
    ...
    report
        ...
        .enumerate()
        .all(|(index, (((binding, hover), type_info), derivation))| {
            ...
            binding.name == hover.name
                && binding.name == type_info.name
                && binding.name == derivation.name
                && binding.line == hover.line
                && binding.line == type_info.line
                && binding.line == derivation.line
                && binding.span.source_id == hover.span.source_id
                && binding.span.source_id == type_info.span.source_id
                && has_expected_owner
                && is_explicit != is_inferred
                ...
                && derivation.expression.as_deref().is_some_and(|expression| {
                    if is_inferred {
                        ...
                        semantic::supports_incremental_annotated_scalar_expression(
                            expression,
...
fn incremental_scalar_prefix_matches_report(
    report: &CheckReport,
    prefix: &ParsedScalarDeclarationPrefix,
    suffix: &ParsedScalarDeclarationSuffix,
    shape: IncrementalScalarReportShape,
) -> bool {
    prefix
        .fast_binding_count
        ...
        && prefix
            ...
            == Some(report.syntax_summary.explicit_declarations)
        && prefix
            .const_declaration_count
        ...
        && prefix
            .imports
            .iter()
            .zip(&report.semantic_program.imports)
            .all(|(declaration, info)| {
...
fn semantic_program_has_only_scalar_declaration_records(
    ...
    if program.imports.len() != import_count
        || program.consts.len() != const_declaration_count
    ...
    program
        ...
        .flat_map(|component| {
            component.local_expressions.iter().flat_map(move |local| {
                component_behavior_calls(&local.expression)
                    .into_iter()
                    .map(move |call| {
                        let behavior_kind = call.behavior_kind;
                        let signal = call.arguments.first().cloned();
                        ComponentBehaviorNode {
                            id: format!("{}.{}:{}", component.name, local.name, behavior_kind),
                            relationship_status: (behavior_kind == "delay")
                                .then(|| BEHAVIOR_RELATIONSHIP_RESOLVED.to_owned()),
                            contract_status: matches!(
                                behavior_kind.as_str(),
                                "predictor" | "external"
                            )
                            .then(|| BEHAVIOR_CONTRACT_RESOLVED.to_owned()),
                            jacobian_policy: (behavior_kind == "predictor")
                                .then(|| BEHAVIOR_JACOBIAN_POLICY.to_owned()),
                            profile_policy: (behavior_kind == "external")
                                .then(|| BEHAVIOR_PROFILE_POLICY.to_owned()),
                            delay_s: (behavior_kind == "delay")
                                .then(|| {
                                    call.arguments.get(1).and_then(|duration| {
                                        review_duration_seconds(duration.trim())
                                    })
                                })
                                .flatten(),
                            behavior_kind,
...
mod tests {
    ...
    #[test]
    fn incrementally_rechecks_scalar_binding_dependencies_with_fresh_equivalence() {
        ...
        let previous_copy_span = previous
            .inferred_declarations
            .iter()
            .find(|declaration| declaration.name == "heat_rate_copy")
            .expect("previous dependent declaration")
            .expression_span;

        let reused =
            recheck_scalar_binding_suffix_incrementally(&previous, previous_source, source)
                .expect("the changed scalar dependency suffix should be rechecked incrementally");
        ...
        let copy_span = reused
            .inferred_declarations
            .iter()
            .find(|declaration| declaration.name == "heat_rate_copy")
            .expect("rechecked dependent declaration")
            .expression_span;
        assert!(copy_span.start > previous_copy_span.start);
        assert_eq!(
            source.get(copy_span.start..copy_span.end),
            Some("heat_rate")
        );

        let heat_rate_type = reused
            .semantic_program
            .typed_bindings
            .iter()
            .find(|binding| binding.name == "heat_rate")
            .expect("changed binding type");
        for name in ["heat_rate_copy", "heat_rate_alias"] {
            let alias_type = reused
                .semantic_program
                .typed_bindings
                .iter()
                .find(|binding| binding.name == name)
                .expect("dependent alias type");
            assert_eq!(alias_type.semantic_type, heat_rate_type.semantic_type);
        }

        let alias_source = "# inputs\r\nheat_rate = 1800 W\r\nratio = 1\r\nheat_rate_copy = ratio\r\nheat_rate_alias = heat_rate_copy\r\n";
        let alias_reused =
            recheck_scalar_binding_suffix_incrementally(&reused, source, alias_source)
                .expect("a backward alias target change should be rechecked incrementally");
        let alias_fresh = check_source(
            "incremental-binding.eng",
            alias_source,
            &CheckOptions::default(),
        );
        assert_eq!(
            alias_reused.inferred_declarations,
            alias_fresh.inferred_declarations
        );
        assert_eq!(alias_reused.syntax_summary, alias_fresh.syntax_summary);
        assert_eq!(alias_reused.semantic_program, alias_fresh.semantic_program);
        assert_eq!(review_json(&alias_reused), review_json(&alias_fresh));
        assert_eq!(
            encode_bytecode(&build_bytecode_program(&alias_reused, alias_source)),
            encode_bytecode(&build_bytecode_program(&alias_fresh, alias_source))
        );
        let ratio_type = alias_reused
            .semantic_program
            .typed_bindings
            .iter()
            .find(|binding| binding.name == "ratio")
            .expect("ratio type");
        let alias_type = alias_reused
            .semantic_program
            .typed_bindings
            .iter()
    ...
    #[test]
    fn incrementally_rechecks_explicit_scalar_declarations_with_fresh_equivalence() {
        fn assert_fresh_equivalent(reused: &CheckReport, fresh: &CheckReport, source: &str) {
            assert_eq!(reused.source_path, fresh.source_path);
            assert_eq!(reused.source_files, fresh.source_files);
            assert_eq!(reused.source_hash, fresh.source_hash);
        ...
        let previous = check_source(
            "incremental-explicit.eng",
            previous_source,
            &CheckOptions::default(),
        );
        assert!(
            previous.diagnostics.is_empty(),
            "unexpected explicit fixture diagnostics: {:#?}",
            previous.diagnostics
        );
        assert_eq!(previous.semantic_program.expected_types.len(), 3);
        assert!(previous.inferred_declarations.is_empty());

        let reused = recheck_explicit_scalar_declaration_suffix_incrementally(
            &previous,
            previous_source,
            source,
        )
        .expect("changed explicit scalar declarations should recheck one suffix");
        let fresh = check_source("incremental-explicit.eng", source, &CheckOptions::default());
        assert_fresh_equivalent(&reused, &fresh, source);
        assert_eq!(
            reused.semantic_program.typed_bindings.first(),
            previous.semantic_program.typed_bindings.first(),
            "the explicit declaration before changed trivia should remain exact"
        );
        let heat_rate = reused
            .semantic_program
            .typed_bindings
            .iter()
            .find(|binding| binding.name == "heat_rate")
            .expect("changed explicit heat-rate binding");
        assert_eq!(heat_rate.semantic_type.display_unit, "W");
        ...
        let appended = recheck_explicit_scalar_declaration_suffix_incrementally(
            &reused,
            source,
            appended_source,
        )
        .expect("an appended explicit scalar should extend every semantic vector");
        let appended_fresh = check_source(
            "incremental-explicit.eng",
            appended_source,
            &CheckOptions::default(),
        ...
        assert_fresh_equivalent(&appended, &appended_fresh, appended_source);
        assert_eq!(appended.syntax_summary.explicit_declarations, 4);

        let cleared_source = "# explicit scalars cleared\r\n";
        let cleared = recheck_explicit_scalar_declaration_suffix_incrementally(
            &appended,
            appended_source,
            cleared_source,
        )
        .expect("removing every explicit scalar should clear its semantic vectors");
        ...
        let restarted_source = concat!(
            "# explicit scalars cleared\r\n",
            "distance: Length [m] = 3 m\r\n",
        );
        let restarted = recheck_explicit_scalar_declaration_suffix_incrementally(
            &cleared,
            cleared_source,
            restarted_source,
        )
        .expect("an explicit scalar should restart a trivia-only report");
    ...
    #[test]
    fn explicit_scalar_incremental_recheck_uses_strict_fallbacks() {
        let previous_source = concat!(
            "length: Length [m] = 2 m\n",
            "heat_rate: HeatRate [kW] = 2 kW\n",
        );
        let previous = check_source(
            "incremental-explicit.eng",
            previous_source,
        ...
        for unsupported in [
            "length: Length [m] = 2 m\nheat_rate: HeatRate [kW] = missing\n",
            "length: Length [m] = 2 m\nheat_rate: HeatRate [kW] = sqrt(4)\n",
            "length: Length [m] = 2 m\nheat_rate = 2 kW\n",
            "length: CustomScalar [m] = 2 m\nheat_rate: HeatRate [kW] = 2 kW\n",
            "length: Length [m]\nheat_rate: HeatRate [kW] = 2 kW\n",
            "length: Length [m] = 2 m\nheat_rate: HeatRate [kW] = 2 kW\n# trailing trivia\n",
            "copy: HeatRate [kW] = heat_rate\nheat_rate: HeatRate [kW] = 2 kW\n",
            "length: Length [m] = 2 m\nlength: HeatRate [kW] = 2 kW\n",
        ] {
            assert!(
                ...
                .is_none(),
                "unsupported explicit edit should use a full check: {unsupported:?}"
            );
        }

        assert!(recheck_explicit_scalar_declaration_suffix_incrementally(
            &previous,
            "length: Length [m] = 20 m\nheat_rate: HeatRate [kW] = 2 kW\n",
            "length: Length [m] = 20 m\nheat_rate: HeatRate [W] = 1800 W\n",
        )
        .is_none());
    }

    #[test]
    fn incrementally_rechecks_mixed_scalar_declarations_with_fresh_equivalence() {
        fn assert_fresh_equivalent(reused: &CheckReport, fresh: &CheckReport, source: &str) {
            assert_eq!(reused.source_path, fresh.source_path);
            assert_eq!(reused.source_files, fresh.source_files);
            assert_eq!(reused.source_hash, fresh.source_hash);
        ...
        let previous_source = concat!(
            "base = 1 m\n",
            "# mixed scalars\n",
            "offset: Length [m] = base + 2 m\n",
            "combined = offset + base + 0 m\n",
            "limit: Length [m] = combined + 1 m\n",
        );
        let source = concat!(
            "base = 1 m\n",
            "# mixed scalar styles changed\n",
            "offset = base + 3 m\n",
            "combined: Length [cm] = offset + base\n",
            "limit = combined + 2 m\n",
        );
        let previous = check_source(
            "incremental-mixed.eng",
            previous_source,
            &CheckOptions::default(),
        );
        assert!(
            previous.diagnostics.is_empty(),
            "unexpected mixed fixture diagnostics: {:#?}",
            previous.diagnostics
        );
        assert_eq!(previous.syntax_summary.fast_bindings, 2);
        assert_eq!(previous.syntax_summary.explicit_declarations, 2);
        let empty_semantic_program = semantic::analyze(&ParsedProgram {
            lines: Vec::new(),
            items: Vec::new(),
        })
        .semantic_program;
        assert!(
            incremental_scalar_report_shape(&previous, 0, &empty_semantic_program).is_some(),
            "the successful mixed report should satisfy the incremental scalar shape"
        );

        let reused =
            recheck_scalar_declaration_suffix_incrementally(&previous, previous_source, source)
                .expect("mixed scalar declaration styles should recheck one suffix");
        let fresh = check_source("incremental-mixed.eng", source, &CheckOptions::default());
        assert_fresh_equivalent(&reused, &fresh, source);
        assert_eq!(
            reused.semantic_program.typed_bindings.first(),
            previous.semantic_program.typed_bindings.first(),
            "the declaration before changed trivia should remain exact"
        );
        assert_eq!(reused.syntax_summary.fast_bindings, 3);
        assert_eq!(reused.syntax_summary.explicit_declarations, 1);
        assert_eq!(reused.inferred_declarations.len(), 3);
        assert_eq!(reused.semantic_program.expected_types.len(), 1);
        assert!(
            recheck_scalar_binding_suffix_incrementally(&previous, previous_source, source)
                .is_none()
        );
        ...
        let appended_source = concat!(
            ...
            "combined: Length [cm] = offset + base\n",
            "limit = combined + 2 m\n",
            "reserve: Length [cm] = limit + 25 cm\n",
            "reserve_copy = reserve\n",
        );
        let appended =
            recheck_scalar_declaration_suffix_incrementally(&reused, source, appended_source)
                .expect("appended mixed declarations should extend all matching semantic vectors");
        let appended_fresh = check_source(
            "incremental-mixed.eng",
            appended_source,
            &CheckOptions::default(),
        ...
        assert_fresh_equivalent(&appended, &appended_fresh, appended_source);
        assert_eq!(appended.syntax_summary.fast_bindings, 4);
        assert_eq!(appended.syntax_summary.explicit_declarations, 2);

        let cleared_source = "# mixed scalars cleared\r\n";
        let cleared = recheck_scalar_declaration_suffix_incrementally(
            &appended,
            appended_source,
            cleared_source,
        )
        .expect("removing mixed declarations should clear every scalar semantic vector");
        let cleared_fresh = check_source(
            "incremental-mixed.eng",
            cleared_source,
            &CheckOptions::default(),
        ...
        .expect("mixed scalar declarations should restart a trivia-only report");
        let restarted_fresh = check_source(
            "incremental-mixed.eng",
            restarted_source,
            &CheckOptions::default(),
    ...
    #[test]
    fn incrementally_rechecks_scalar_suffix_after_supported_stdlib_imports() {
        fn assert_fresh_equivalent(reused: &CheckReport, fresh: &CheckReport, source: &str) {
            assert_eq!(reused.source_path, fresh.source_path);
            assert_eq!(reused.source_files, fresh.source_files);
            assert_eq!(reused.source_hash, fresh.source_hash);
        ...
        let previous_source = concat!(
            "use eng.stats\r\n",
            "import eng.path\r\n",
            "# imported scalar inputs\r\n",
            "base = 2 m\r\n",
            "const factor: Ratio [1] = 0.5\r\n",
        ...
        let previous = check_source(
            "incremental-stdlib.eng",
            previous_source,
            &CheckOptions::default(),
        );
        assert!(
            previous.diagnostics.is_empty(),
            "unexpected stdlib fixture diagnostics: {:#?}",
            previous.diagnostics
        );
        assert_eq!(previous.syntax_summary.imports, 2);
        assert_eq!(previous.semantic_program.imports.len(), 2);
        let mut mismatched_import_report = previous.clone();
        mismatched_import_report.semantic_program.imports[0].status = "stale".to_owned();
        assert!(recheck_scalar_declaration_suffix_incrementally(
            &mismatched_import_report,
            previous_source,
            source,
        )
        .is_none());
        ...
        let reused =
            recheck_scalar_declaration_suffix_incrementally(&previous, previous_source, source)
                .expect("supported stdlib imports should preserve the scalar suffix fast path");
        let fresh = check_source("incremental-stdlib.eng", source, &CheckOptions::default());
        assert_fresh_equivalent(&reused, &fresh, source);
        assert_eq!(
            reused.semantic_program.imports, previous.semantic_program.imports,
            "unchanged stdlib import records should remain exact"
        );
        assert_eq!(reused.syntax_summary.imports, 2);
        ...
        let cleared =
            recheck_scalar_declaration_suffix_incrementally(&reused, source, cleared_source)
                .expect("clearing declarations should preserve supported stdlib imports");
        let cleared_fresh = check_source(
            "incremental-stdlib.eng",
            cleared_source,
            &CheckOptions::default(),
        ...
        assert_fresh_equivalent(&cleared, &cleared_fresh, cleared_source);
        assert!(cleared.semantic_program.typed_bindings.is_empty());
        assert_eq!(cleared.semantic_program.imports.len(), 2);
        assert_eq!(cleared.semantic_program.workflow.line, 1);
        ...
        .expect("declarations should restart after unchanged supported stdlib imports");
        let restarted_fresh = check_source(
            "incremental-stdlib.eng",
            restarted_source,
            &CheckOptions::default(),
        ...
        assert_fresh_equivalent(&restarted, &restarted_fresh, restarted_source);
        assert_eq!(restarted.semantic_program.workflow.line, 4);
    }
    ...
    fn incrementally_rechecks_scalar_suffix_after_static_scalar_imports() {
        ...
        let root = std::env::temp_dir().join(format!(
            "eng_compiler_incremental_static_const_import_{}",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&root).expect("file import fixture should be created");
        std::fs::write(
            root.join("base.eng"),
            "const imported_length: Length [m] = 2 m\n",
        )
...
const imported_adjusted: Length [m] = add_lengths(add_lengths(imported_length, imported_length), imported_length) * imported_factor
"#,
        )
...
base = add_lengths(add_lengths(imported_adjusted, imported_length), imported_length) + imported_length
const local_factor: Ratio [1] = 0.5
adjusted: Length [m] = add_lengths(add_lengths(base, imported_length), imported_length) * local_factor
"#;
...
adjusted: Length [cm] = add_lengths(add_lengths(base, imported_length), imported_length) * local_factor
total = add_lengths(add_lengths(adjusted, imported_length), imported_length) + add_lengths(base, imported_length)
"#;
        ...
        assert!(
            previous.diagnostics.is_empty(),
            "unexpected file import diagnostics: {:#?}",
            previous.diagnostics
        );
        ...
        assert!(semantic::supports_incremental_fast_binding_expression(
            "add_lengths(add_lengths(imported_adjusted, imported_length), imported_length) + imported_length",
        ...
        let previous_suffix = parse_scalar_declaration_suffix(&previous_lines[4..])
            .expect("changed scalar suffix should parse");
        let empty_semantic_program = semantic::analyze(&ParsedProgram {
            lines: Vec::new(),
            items: Vec::new(),
        })
        ...
        assert!(semantic::supports_incremental_annotated_scalar_expression(
            "add_lengths(add_lengths(base, imported_length), imported_length) * local_factor",
            "Length",
            &previous.semantic_program.typed_bindings[..5],
            &previous.semantic_program.functions,
        ));
        assert!(semantic::supports_incremental_fast_binding_expression(
            "add_lengths(add_lengths(adjusted, imported_length), imported_length) + add_lengths(base, imported_length)",
        ...
        .is_none());
        let mut stale_owner = previous.clone();
        stale_owner.semantic_program.consts[0].span.source_id = SourceSpan::ROOT_SOURCE_ID;
        assert!(recheck_scalar_declaration_suffix_incrementally(
            &stale_owner,
            previous_source,
            source,
        ...
        assert_fresh_equivalent(&reused, &fresh, source);
        assert_eq!(
            &reused.semantic_program.consts[..3],
            &previous.semantic_program.consts[..3]
        );
        assert_eq!(
...
"#;
        let cleared =
            recheck_scalar_declaration_suffix_incrementally(&reused, source, cleared_source)
                .expect("clearing root declarations should retain imported constants");
...
use eng.stats
# root scalar declarations cleared
copied_length = add_lengths(add_lengths(imported_adjusted, imported_length), imported_length)
result: Length [m] = add_lengths(add_lengths(copied_length, imported_length), imported_length) + imported_length
"#;
        let restarted = recheck_scalar_declaration_suffix_incrementally(
            &cleared,
            cleared_source,
        ...
        .expect("root declarations should restart against imported constants");
...
fast_result = add_lengths(add_lengths(imported_adjusted, imported_length), imported_length)
"#;
        let fast_source = r#"use "shared.eng"
fast_result = add_lengths(add_lengths(imported_length, imported_length), imported_adjusted) + imported_length
"#;
        let fast_previous =
            check_source(&main_path, fast_previous_source, &CheckOptions::default());
        let fast_reused = recheck_scalar_binding_suffix_incrementally(
            &fast_previous,
        ...
        for unsupported_call in [
            ...
            previous_source.replace(
                "add_lengths(imported_adjusted, imported_length)",
                "add_lengths(missing_length(imported_adjusted), imported_length)",
            ),
            previous_source.replace(
                "add_lengths(add_lengths(imported_adjusted, imported_length), imported_length) + imported_length",
                "add_lengths(add_lengths(imported_adjusted, imported_length), imported_length) + * imported_length",
            ...
            assert!(
                ...
                .is_none(),
                "invalid scalar function call should use a full check: {unsupported_call:?}"
            );
        }

        let explicit_previous_source = r#"use "shared.eng"
explicit_result: Length [m] = add_lengths(add_lengths(imported_adjusted, imported_length), imported_length) + imported_length
"#;
        let explicit_source = r#"use "shared.eng"
explicit_result: Length [cm] = add_lengths(add_lengths(imported_length, imported_length), imported_adjusted) + imported_length
"#;
        ...
        let explicit_reused = recheck_explicit_scalar_declaration_suffix_incrementally(
            &explicit_previous,
            explicit_previous_source,
            explicit_source,
        )
        ...
        for unsupported_explicit_call in [
            ...
            explicit_previous_source.replace(
                "add_lengths(add_lengths(imported_adjusted, imported_length), imported_length) + imported_length",
                "add_lengths(add_lengths(imported_adjusted, imported_length), imported_length) + imported_factor",
            ),
            explicit_previous_source.replace(
                "add_lengths(imported_adjusted, imported_length)",
                "add_lengths(missing_length(imported_adjusted), imported_length)",
            ...
            assert!(
                ...
                .is_none(),
                "invalid explicit scalar call should use a full check: {unsupported_explicit_call:?}"
            );
        }

        let const_call_previous_source = r#"use "shared.eng"
const const_result: Length [m] = add_lengths(add_lengths(imported_adjusted, imported_length), imported_length) + imported_length
"#;
        let const_call_source = r#"use "shared.eng"
const const_result: Length [cm] = add_lengths(add_lengths(imported_length, imported_length), imported_adjusted) + imported_length
"#;
        ...
        let const_call_reused = recheck_scalar_declaration_suffix_incrementally(
            &const_call_previous,
            const_call_previous_source,
            const_call_source,
        )
        .expect("root scalar const declarations should reuse imported scalar functions");
        let const_call_fresh =
            check_source(&main_path, const_call_source, &CheckOptions::default());
        assert_fresh_equivalent(&const_call_reused, &const_call_fresh, const_call_source);

        for unsupported_const_call in [
            const_call_previous_source.replace(
                "add_lengths(imported_adjusted, imported_length)",
                "add_lengths(imported_adjusted)",
            ),
            const_call_previous_source.replace(
                "const const_result: Length [m]",
            ...
            assert!(
                recheck_scalar_declaration_suffix_incrementally(
                    &const_call_previous,
                    const_call_previous_source,
                    &unsupported_const_call,
                )
                .is_none(),
                "invalid const scalar call should use a full check: {unsupported_const_call:?}"
            );
        }
    ...
    fn scalar_const_incremental_recheck_uses_strict_fallbacks() {
        ...
        let imported = check_source(
            "incremental-import-edit.eng",
            imported_source,
            &CheckOptions::default(),
        );
        ...
        assert!(recheck_scalar_declaration_suffix_incrementally(
            &imported,
            imported_source,
            &imported_source.replace("eng.stats", "eng.path"),
        )
        .is_none());

        for unsupported_prefix in ["use eng.building\n", "use missing.eng\n"] {
            let previous_source = format!("{unsupported_prefix}base = 2 m\nfactor = 0.5\n");
            let source = format!("{unsupported_prefix}base = 2 m\nfactor = 0.75\n");
            let previous = check_source(
                "incremental-unsupported-import.eng",
                &previous_source,
                &CheckOptions::default(),
            );
        ...
        let root = std::env::temp_dir().join(format!(
            "eng_compiler_incremental_file_import_{}",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&root).expect("file import fixture should be created");
        ...
        .expect("file import module should be written");
        let main_path = root.join("main.eng");
        let file_import_source = r#"use "shared.eng"
base = 2 m
factor = 0.5
"#;
        std::fs::write(&main_path, file_import_source).expect("file import root should be written");
        let file_import_report =
            check_source(&main_path, file_import_source, &CheckOptions::default());
        assert!(
            file_import_report.diagnostics.is_empty(),
            "unexpected file import diagnostics: {:#?}",
    ...
    #[test]
    fn mixed_scalar_incremental_recheck_uses_strict_fallbacks() {
        let previous_source = concat!(
            "length = 2 m\n",
            "copy: Length [m] = length\n",
            "combined = copy + length + 0 m\n",
        );
        let previous = check_source(
            "incremental-mixed.eng",
            previous_source,
        ...
        for unsupported in [
            "length = 2 m\ncopy: Length [m] = missing\ncombined = copy + length + 0 m\n",
            "length = 2 m\ncopy: Length [m] = sqrt(4)\ncombined = copy + length + 0 m\n",
            "length = 2 m\ncopy: Length [m] = length + 1 s\ncombined = copy + length + 0 m\n",
            "copy: Length [m] = length\nlength = 2 m\ncombined = copy + length + 0 m\n",
            "length = 2 m\nlength: Length [m] = 3 m\ncombined = length + 1 m\n",
            "length = 2 m\ncopy: CustomScalar [m] = length\ncombined = copy + length + 0 m\n",
            "length = 2 m\ncopy: Length [m]\ncombined = copy + length + 0 m\n",
            "length = 2 m\nprint length\ncombined = length + 1 m\n",
            "use \"other.eng\"\nlength = 2 m\ncombined: Length [m] = length + 1 m\n",
            "length = 2 m\ncopy: Length [m] = length\ncombined = copy + length + 0 m\n# trailing trivia\n",
        ] {
            assert!(
                recheck_scalar_declaration_suffix_incrementally(
                ...
                .is_none(),
                "unsupported mixed edit should use a full check: {unsupported:?}"
            );
        }
    ...
    fn scalar_binding_incremental_recheck_uses_strict_fallbacks() {
        ...
        let richer_source = "length = 2 m\nprint length\nheat_rate = 2 kW\n";
        let richer = check_source(
            "incremental-binding.eng",
            richer_source,
            &CheckOptions::default(),
        );
...
"#;
    ...
    fn records_domain_component_and_connection_metadata() {
        ...
        assert!(review.contains("\"declared_not_executed\""));
        ...
        assert!(review.contains("\"relationship_status\": \"relationship_resolved\""));
...
"#;
    ...
    #[test]
    fn records_predictor_declared_status_in_component_preview() {
        let report = check_source(
            "ok.eng",
        ...
        assert_eq!(assembly.solver_preview.predictor, BEHAVIOR_STATUS_DECLARED);
    }

    #[test]
    fn records_external_behavior_declared_status_in_component_preview() {
        let report = check_source(
            "ok.eng",
    ...
    fn infers_scalar_arithmetic_around_valid_function_calls() {
        let report = check_source(
            ...
            concat!(
                ...
                "base = 2 m\n",
                "offset = 1 m\n",
                "nested = add_lengths(add_lengths(base, offset), offset)\n",
                "combined = add_lengths(add_lengths(base, offset), offset) + add_lengths(offset, base)\n",
                "scaled = add_lengths(add_lengths(base, offset), offset) * 0.5\n",
            ),
            &CheckOptions::default(),
        );

        assert!(!report.has_errors(), "{:#?}", report.diagnostics);
        for name in ["nested", "combined", "scaled"] {
            let declaration = report
                .inferred_declarations
                .iter()
                .find(|declaration| declaration.name == name)
