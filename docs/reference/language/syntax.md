...
| Raw bindings are strings | `read json/toml` records source text until the binding is promoted through a schema |
...
| `E-BLOCK-BINDING-001` | Block/declaration/member/metadata header used as a bound value | Write headers at statement start or attach `where`/`with` to a supported owner |
...
| `E-OPTION-BINDING-001` | Workflow option assignment used as a bound value | Put options inside a `with { ... }` block attached to the statement they configure |
