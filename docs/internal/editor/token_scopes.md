...
| `support.function.timeseries.englang` | TimeSeries/statistic helper calls such as `integrate(...)`, `mean(...)`, `min(...)`, `max(...)`, `sum(...)`, `time_weighted_mean(...)`, and `p90(...)`. |
