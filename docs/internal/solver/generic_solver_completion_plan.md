...
This is an advanced/internal implementation ledger, not product identity and
not public package scope.
...
| Fixed-point solve | Source `solve component_graph` works for pivotable linear ResidualGraph loops and direct expression-mapped `x = g(...)` residuals where one side of an `eq` is a single unknown. | Partitioning is still direct-equation only; broader coupled mapping extraction and convergence policy remain limited. | Fixed-point mapping extraction works from shared residual IR with relaxation policy, variable scaling, residual history, and nonconvergence artifacts. |
...
- 2026-06-23: Fixed-point step diagnostics now name the source residual that drives each update instead of only naming the updated variable, allowing report-spec largest-residual source context to resolve for fixed-point iteration history. Runtime and CLI fixture `tests/runtime/source_system_fixed_point_coupled_affine_side.eng` also pins a static source-system mapping where the solved variable shares an affine equation side with another variable (`x + 0.25 * y eq cos(y)`), preserving source residual context for both update equations. This advances the fixed-point residual-history and partition-evidence gate without claiming arbitrary nonlinear partition extraction.

- 2026-06-23: Static source-system fixed-point nonconvergence is now pinned as an explicit diagnostics artifact instead of relying only on component-graph evidence. `tests/diagnostics/source_system_fixed_point_nonconvergence.eng` proves `solve <SystemName> with { solver = fixed_point }` preserves `fixed_point_not_converged`, `E-FIXED-POINT-NONCONVERGENCE`, per-step residual vectors, variable-scale policy, final residual evidence, `largest_residuals`, report-spec JSON, and report HTML through runtime and CLI smoke coverage. This closes a fixed-point failure-artifact gap while keeping broad partition extraction and event/adaptive coupling planned.

- 2026-06-22: Fixed-point source solves now materialize user-provided `variable_scale`/`variable_scales` options instead of only reporting unit-default update scales. Runtime and CLI fixture `tests/runtime/source_system_fixed_point_variable_scales.eng` proves static source-system fixed-point solves preserve `user_provided_variable_scales`, `variable_scale_min`, `variable_scale_max`, and per-step variable-scale policy in `.engres` and report-spec JSON. This advances the fixed-point scale-policy closure gate while keeping the direct/affine expression-mapping partition limitation explicit.

- 2026-06-22: Static source-system Newton nonconvergence is now explicit rather than appearing as a partial success. Diagnostics fixture `tests/diagnostics/source_system_newton_nonconvergence.eng` proves `solve <SystemName>` preserves `newton_not_converged`, `E-NEWTON-NONCONVERGENCE`, per-step failure artifacts, and `largest_residuals` through runtime `.engres`, report-spec JSON, report HTML, and CLI smoke coverage. This extends Newton failure evidence from component residual graphs to static source systems without claiming broad nonlinear partitioning or symbolic Jacobians.

- 2026-06-22: Static source-system Newton now has source-linear provided-Jacobian evidence through `solve <SystemName> with { solver = newton; jacobian = source_linear_terms }`. Runtime and CLI smoke fixture `tests/runtime/source_system_newton_source_linear_jacobian.eng` proves the source-system residual graph reuses the Newton provided-Jacobian path, records `newton_source_residual_graph_with_provided_jacobian`, preserves `jacobian_policy = source_linear_terms` in diagnostics/report-spec JSON, and solves the two-variable source system without switching to dense-linear solve.

- 2026-06-22: Static source-system Newton solves now accept user-provided `with { variable_scale = ... }` or `variable_scales = [...]` options. The runtime materializes the scales through the source-system residual graph, records `user_provided_variable_scales`, `variable_scale_min`, and `variable_scale_max` in `.engres`/report-spec artifacts, and proves the path through runtime plus CLI coverage in `tests/runtime/source_system_newton_variable_scales.eng`. This closes user-scale propagation for the static source Newton path without claiming broad DAE/component scale-policy completion.

- 2026-06-22: Source `solve component_graph` fixed-point updates now fall back from pivotable linear residual terms to parsed source-expression mappings when an equation side exposes a direct or affine target, such as `loop.source.x eq cos(loop.target.x)` or `2 * x + offset eq cos(y)`. The bridge evaluates final residual artifacts through the parsed source residual AST, keeps user-facing residual tolerance aligned with a stricter internal update tolerance, and adds runtime/CLI fixture coverage in `tests/runtime/fixed_point_expression_mapping.eng` and `tests/runtime/fixed_point_affine_expression_mapping.eng` for solved variables, residual expressions, convergence status, report-spec JSON, and report HTML. This broadens fixed-point evidence beyond linear pivots without claiming arbitrary partitioning or a general fixed-point extraction policy.

- 2026-06-22: Static algebraic source systems now lower directly into the residual-graph solve path through `solve <SystemName>`. Square systems without `der(...)` equations support `dense_linear`/`linear` for linear residuals, `fixed_point` for direct or affine single-target `eq` side mappings, and `newton`/`nonlinear_newton` for nonlinear residuals, treating source `state`/`output` variables as static algebraic unknowns and parameter/scalar input defaults as residual constants. Runtime unit tests, CLI smoke fixtures `tests/runtime/source_system_dense_linear_solve.eng`, `tests/runtime/source_system_fixed_point_solve.eng`, `tests/runtime/source_system_fixed_point_coupled_affine_side.eng`, `tests/runtime/source_system_fixed_point_variable_scales.eng`, `tests/runtime/source_system_fixed_point_affine_solve.eng`, and `tests/runtime/source_system_newton_solve.eng`, `tests/runtime/source_system_newton_source_linear_jacobian.eng`, plus `tests/runtime/source_system_newton_variable_scales.eng`, while `tests/diagnostics/source_system_fixed_point_nonconvergence.eng` and `tests/diagnostics/source_system_newton_nonconvergence.eng` prove explicit nonconvergence failure artifacts; report-spec JSON and report HTML prove solved variables, residuals, iteration diagnostics, source lines, Jacobian policy, variable-scale policy, convergence status, and failure codes. This advances generic equation-system work while leaving dynamic, DAE, event, component-coupled, and broad unit-expression solving in the planned lanes.

- 2026-06-22: System solver `source_equations` are now part of the result/report/review schema contract and are summarized in report/IDE system solver inspection surfaces. Example smoke now checks that source-equation evidence reaches report HTML for source ODE success/failure and one-state thermal scalar-input workflows, closing another inspection gap without claiming broad shared-IR completion.
- 2026-06-22: Unsupported system-shape artifacts now preserve user-authored system equations as `unsupported_system_equation` source metadata when equations exist, while equation-less skipped artifacts remain explicit empty lists. This makes unsupported solver boundaries inspectable without presenting the skipped path as a solved general equation-system workflow.
- 2026-06-22: Source-equation ODE failure artifacts now retain shared `source_equations` metadata for output-evaluator creation, RHS-evaluator creation, fixed/adaptive solver evaluation, and output-evaluation failure branches. The output algebraic-loop diagnostics fixture now proves the failed runtime/report/review artifacts still expose the source derivative equation metadata, improving failure inspection without broadening the supported source-equation shape.
- 2026-06-22: One-state first-order thermal solves now attach shared `source_equations` metadata to runtime `.engres`, report-spec, and review artifacts, including the recognized heat-balance equation kind, target state, left/right expressions, residual expression, source line, and HeatRate unit metadata. Runtime tests cover fixed-step, adaptive Heun, and failed thermal model validation paths, while example-smoke coverage proves the metadata in the scalar-input override artifact; this reduces special-runner opacity without claiming a general equation-system solver.
- 2026-06-22: Typed-block state-space solves now attach shared `source_equations` metadata to runtime `.engres`, report-spec, and review artifacts. The adapter exposes A/B operator declarations plus row-level `state_space_update` or `state_space_derivative` equations with source lines, target states, residual expressions, quantity kinds, and display/canonical units, alongside algebraic output equations. Runtime unit tests and official state-space example smoke coverage prove the metadata for discrete and continuous typed-block state-space workflows; broad operator algebra, nonlinear/DAE composition, and component-coupled state-space remain planned.
- 2026-06-22: Source-equation ODE lowering now carries derivative and algebraic-output source equation metadata into runtime `.engres`, report-spec, and review artifacts. Each materialized source ODE solution exposes `source_equations` entries with equation kind, target, left/right expressions, residual expression, source line, quantity kind, display unit, and canonical unit. Runtime/example-smoke coverage proves derivative and output equation metadata for `tests/runtime/source_ode_output_dependency.eng`; this advances shared-IR/report visibility work without claiming a general equation-system solver.
- 2026-06-22: `simulate` with-block handling now validates declared scalar system input override values as finite numeric literals with compatible units, materializes those values into canonical `SolverInput.inputs`, and uses them consistently in source-equation ODE, typed-block state-space, one-state thermal RHS evaluation, and source output materialization. Runtime fixtures `tests/runtime/source_ode_scalar_input_override.eng`, `tests/runtime/state_space_scalar_input_override.eng`, and `tests/runtime/thermal_scalar_input_override.eng` prove scalar input overrides change source-equation, typed-block state-space, and one-state thermal trajectories; broader non-literal input bindings and general equation-system input lowering remain planned.
- 2026-06-21: `simulate` with-block handling now accepts declared system parameter names as numeric overrides, validates finite literal/unit compatibility in compiler diagnostics, and materializes override values into `SolverInput.parameters` for one-state thermal, source-equation ODE, and state-space simulation paths. Runtime fixture `tests/runtime/source_ode_parameter_override.eng` proves `gain = 2` changes source RHS and output trajectories; broader args/object/non-literal parameter overrides remain planned.
- 2026-06-21: Source-equation ODE scalar output materialization now topologically orders acyclic output-to-output dependencies before evaluating per-sample trajectories. Runtime fixture `tests/runtime/source_ode_output_dependency.eng` proves a declared-later helper output can feed another output, and diagnostics fixture `tests/diagnostics/source_ode_output_algebraic_loop.eng` proves cycles fail explicitly as `E-RHS-OUTPUT-ALGEBRAIC-LOOP` failure artifacts. This advances source output lowering without claiming broad equation-system, DAE, behavior, or component-coupled output solving is complete.
- 2026-06-21: `simulate` with-block validation now admits arbitrary declared system input names for the targeted `simulate <System>` owner, instead of hard-coding names such as `T_out`. Official example `examples/advanced_solver/34_three_state_source_ode` and runtime fixture `tests/runtime/source_ode_three_state_nonthermal.eng` prove a three-state non-thermal source-equation ODE with a Time-indexed DimensionlessNumber input, adaptive Heun substep diagnostics, and scalar output TimeSeries materialization. This removes the previous evidence-only two-state ceiling for source-equation ODE workflows without claiming broad equation-system, nonlinear, DAE, or component-coupled solving is complete.
- 2026-06-21: Dynamic component derivative Newton fallback and selected algebraic Newton fallback now consume `with { residual_scale = ... }` and `with { residual_scales = [...] }` overrides for named source residuals. Runtime fixtures `tests/runtime/dynamic_component_derivative_residual_scale_override.eng`, `tests/runtime/dynamic_component_algebraic_residual_scale_override.eng`, and `tests/runtime/dynamic_component_adaptive_algebraic_residual_scale_override.eng` prove Newton diagnostics preserve raw residual values separately from normalized values, while diagnostics fixtures prove invalid and affine-derivative dynamic residual scale requests fail explicitly with `E-SOURCE-RESIDUAL-SCALE`. This broadens W4 residual scaling evidence into selected dynamic component Newton solving without claiming broad component nonlinear solving or a complete scale-policy language.
- 2026-06-21: Source `solve component_graph` dense linear, fixed-point, Newton, and implicit-Euler DAE residual evaluations now accept user-provided residual scale overrides through `with { residual_scale = residual = value unit }` or `with { residual_scales = [...] }`. The dense linear and fixed-point fixtures prove final residual metadata and report-spec artifacts use override-aware normalized values and scale policies; the source Newton fixture proves overridden residual scales affect source-linear Jacobian scaling, raw/normalized residual vectors, final residual metadata, and report-spec artifacts; the DAE fixture proves per-step raw residual vectors are reconstructed from normalized solver history while preserving normalized vectors and user-provided scale policies; the diagnostics fixtures prove invalid scales surface explicit `E-SOURCE-RESIDUAL-SCALE` failure artifacts. This advances W4 residual scaling evidence without claiming broad nonlinear/DAE solving or a general scale-policy language.
...
| --- | --- | --- | --- |
| General equation-system runtime is beyond the supported source-equation ODE shape, even though source ODE evidence now extends past one/two-state cases. | Lower arbitrary checked system equations into the shared solver IR, build named `x`, `u(t)`, `p`, `z`, output, and derivative layouts, and evaluate RHS/residual expressions through the typed expression tree. | Official multi-state non-thermal source example plus diagnostics for missing/duplicate derivatives, unsupported algebraic loops, unit mismatches, non-finite RHS, and generated RuntimeTimeSeries for every state/output. Current evidence includes static algebraic source-system solves in `tests/runtime/source_system_dense_linear_solve.eng`, `tests/runtime/source_system_fixed_point_solve.eng`, `tests/runtime/source_system_fixed_point_coupled_affine_side.eng`, `tests/runtime/source_system_fixed_point_variable_scales.eng`, `tests/runtime/source_system_fixed_point_affine_solve.eng`, `tests/diagnostics/source_system_fixed_point_nonconvergence.eng`, `tests/runtime/source_system_newton_solve.eng`, `tests/runtime/source_system_newton_source_linear_jacobian.eng`, `tests/runtime/source_system_newton_variable_scales.eng`, and `tests/diagnostics/source_system_newton_nonconvergence.eng`, plus source ODE evidence from `examples/advanced_solver/34_three_state_source_ode`, `tests/runtime/source_ode_three_state_nonthermal.eng`, `tests/runtime/source_ode_parameter_override.eng`, `tests/runtime/source_ode_scalar_input_override.eng`, `tests/runtime/state_space_scalar_input_override.eng`, `tests/runtime/thermal_scalar_input_override.eng`, `tests/runtime/source_ode_output_dependency.eng`, and `tests/diagnostics/source_ode_output_algebraic_loop.eng`. | Keep expanding the source-system `EquationAssembly` adapter into a shared system-IR adapter; add source spans and unit metadata to every lowered static residual, derivative, and algebraic output equation. |
...
| Component equations are limited to literal boundary constraints, simple linear forms, and selected unit-parameterized coefficients. | Lower component-local arithmetic, derivative, behavior, nonlinear functions, affine units, compound units, and parameter values through one typed expression API. | Linear, dynamic, nonlinear, and DAE component examples all consume the same expression tree; unsupported unit algebra fails with source diagnostics instead of string-shape fallback. | Continue replacing one-off residual parsing with reusable expression AST metadata and evaluator entrypoints. |
...
| Fixed-point source solve is no longer pivotable-linear only, but expression mapping is limited to direct or affine single-target `eq` sides. | Detect general `x = g(x)` partitions from shared residual IR, add variable scaling, relaxation policy, residual history, and nonconvergence artifacts. | Success/failure fixtures cover coupled fixed-point loops, bad partitions, non-finite updates, tolerance/max-iteration options, and report/IDE residual history. Current expression-mapping evidence includes component-graph coverage in `tests/runtime/fixed_point_expression_mapping.eng` and `tests/runtime/fixed_point_affine_expression_mapping.eng`, direct static source-system coverage in `tests/runtime/source_system_fixed_point_solve.eng`, coupled affine-side static source-system coverage in `tests/runtime/source_system_fixed_point_coupled_affine_side.eng`, static source-system user-scale coverage in `tests/runtime/source_system_fixed_point_variable_scales.eng`, static source-system nonconvergence coverage in `tests/diagnostics/source_system_fixed_point_nonconvergence.eng`, and affine static source-system coverage in `tests/runtime/source_system_fixed_point_affine_solve.eng`. | Move fixed-point update extraction from residual-shape helper to shared residual IR metadata. |
| Newton source solve is no longer limited to component residual targets, but it is still a constrained static algebraic residual path. | Evaluate broad nonlinear residual trees from shared IR, add variable scales, Jacobian policy, line-search/trust-region diagnostics, finite-difference fallback, and source-line residual evidence across source systems and component assemblies. | Multi-variable unitful nonlinear source/component examples with nonlinear operators, singular/ill-conditioned/nonconvergent cases, and report/IDE Jacobian/residual diagnostics. Current source-system evidence includes `tests/runtime/source_system_newton_solve.eng`, `tests/runtime/source_system_newton_source_linear_jacobian.eng`, `tests/runtime/source_system_newton_variable_scales.eng`, and `tests/diagnostics/source_system_newton_nonconvergence.eng`. | Use parsed residual ASTs for all Newton evaluations and surface inferred expression units in solver artifacts. |
...
- Keep variable scale policy, source residual scale overrides, and largest-normalized-residual summaries wired through runtime/report artifacts; broaden this beyond the current source dense-linear/fixed-point/Newton/DAE and selected dynamic derivative Newton residual paths.
...
- Emit RuntimeTimeSeries for every named state and selected algebraic output. Source-equation scalar outputs can now depend on other acyclic scalar outputs and reject output dependency cycles with `E-RHS-OUTPUT-ALGEBRAIC-LOOP`; typed-block state-space scalar outputs are implemented; selected explicit and semi-implicit dynamic-component algebraic output fixtures are implemented; broad shared-IR/component-coupled outputs remain.
...
### W8. Production Multi-Domain Foundations
...
- Unit-parameterized linear component equations such as `Q eq UA * (T1 - T2)` across compatible display units, with compiler dimension checks, runtime coefficient conversion, source-to-runtime tests, docs, and `examples/advanced_solver/33_unit_parameterized_wall` example-smoke coverage.
...
- Simulate parameter options now accept declared system parameter names, validate numeric literal/unit compatibility, and materialize canonical solver parameter values for one-state thermal, source-equation ODE, and state-space simulation paths.
- Simulate scalar input options now accept declared scalar system input names, validate finite numeric literal/unit compatibility, and materialize canonical solver input values for one-state thermal, source-equation ODE, and state-space simulation paths.
...
- Source-equation ODE scalar output materialization now topologically orders acyclic output-to-output dependencies, so computed output samples can feed later dependent outputs, and output algebraic loops surface explicit `E-RHS-OUTPUT-ALGEBRAIC-LOOP` failure artifacts.
...
- Fixed-point solver results now retain per-iteration update residual vectors, and source fixed-point component artifacts expose raw/normalized variable-update vectors, variable-scale policy, and largest update identity in runtime `.engres` and report-spec step diagnostics. Source fixed-point component solves also support direct parsed-expression mappings such as `x eq cos(y)` when one `eq` side is a single unknown, with final residuals evaluated through the parsed source residual AST.
