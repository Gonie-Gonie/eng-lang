...
- Added `EngLang: Show Tooling Status`, which opens a JSON status document with
  ...
  `eng-lsp.exe` paths, diagnostics mode, lint toggles, semantic highlighting,
...
## Batch 434: Native Workflow Python Guard Expansion

Status: implemented after re-checking workflow 01/02/03 entrypoints and public workflow docs for Python/process assumptions.

- Expanded `workflows-test` source guards to reject direct `py` launcher, pip/conda/venv/ipython/pytest markers, and additional Python data/ML stack markers such as `pyarrow`, `xarray`, `tensorflow`, `pytorch`, and `torch`.
- Mirrored the expanded Python/tooling marker list in the Rust example smoke guard so packaged example checks cannot drift from the PowerShell workflow gate.
- Added public-doc wording guards for `generated by Python`, `Python-generated`, `Python-made`, `Python-backed`, and `Python-side` so workflow 01/02/03 docs keep describing native artifact materialization.

## Batch 435: VS Code Diagnostics Mode Wording

Status: implemented after reviewing user-facing VS Code command and Settings wording for linter usability.

- Renamed the visible command from `EngLang: Switch Problems Source...` to `EngLang: Switch Diagnostics Mode...` while keeping the existing `englang.problemsSource` setting key as the compatibility surface.
...
## Batch 472: LSP Unterminated Interpolation Quick Fix Parity

Status: implemented after comparing string interpolation local quick fixes with eng-lsp code actions.
...
## Batches 475-528: Compact Current-Backlog Archive

Status: archived from the current usability backlog to keep the active file focused on open work and the most recent changes.
...
- Batch 523: Extended Rust workflow native-only guards to every workflow `.eng` source.
...
## Batches 529-570: Compact Current-Backlog Archive
...
## Batches 571-650: Compact Current-Backlog Archive
...
- Batch 849: Added `EngLang: Copy Highlight Token at Cursor` so VS Code users can copy current or nearest same-line role-aware highlight token details directly from the editor context menu.
