...
| SOLVER-406 | Dynamic component adaptive Heun source path | Implemented narrowly for parsed derivative residual component graphs as `solver = dynamic_component_adaptive_heun`, with scalar or fixed-step TimeSeries component inputs, selected nonlinear derivative residual Newton fallback including a combined TimeSeries-driven derivative Newton fixture, selected affine/Newton algebraic output materialization including a combined TimeSeries-driven Newton fixture, fixed output-grid trajectories, accepted-substep diagnostics, output-grid Newton residual diagnostics, and explicit Newton nonconvergence artifacts. |
...
| Residuals are computed and inspectable | Component solver residuals, normalized residuals, user-provided source residual scale policy metadata, largest residuals, and residual dependency graph inspectors. |
