...
| SOLVER-203 | Discrete state-space runtime | Implemented for `examples/official/21_state_space_discrete` and internal runtime seeds, including scalar output TimeSeries materialization when `outputs y = [...]` names a system `output`. |
| SOLVER-204 | Continuous state-space via RHS evaluator | Implemented for `examples/official/22_state_space_continuous` and internal runtime seeds, including scalar output TimeSeries materialization after fixed-step or adaptive state-space solves. |
