...
| SOLVER-103 | Multi-state RHS evaluator | Implemented for the supported source-equation ODE shape beyond two-state evidence, including arbitrary declared simulate input option names, TimeSeries input materialization, numeric system parameter override materialization, scalar algebraic output TimeSeries materialization with acyclic output-to-output dependency ordering and explicit loop failure artifacts, and internal state-space paths; broad general equation solving remains planned. |
