...
| SOLVER-406 | Dynamic component adaptive Heun source path | Implemented narrowly for parsed derivative residual component graphs as `solver = dynamic_component_adaptive_heun`, with scalar or fixed-step TimeSeries component inputs, affine algebraic output materialization, fixed output-grid trajectories, accepted-substep diagnostics, and explicit unsupported artifacts for non-affine algebraic dynamic component graphs. |
