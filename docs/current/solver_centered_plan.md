...
| v1.7 nonlinear/delay/Predictor integration | Nonlinear seed, delay buffer, Predictor contract | Adaptive Heun is wired into the one-state thermal path and internal continuous state-space path; narrow source Newton/implicit-Euler DAE bridges are wired into component residual assemblies; behavior integration remains solver-API/report/IDE seed scope, with broader language-level integration planned. |
