...
- `SolverInput::validate_layouts` now also guards solver method/timestep,
  tolerance, max-iteration, and TimeGrid consistency before any solver
  algorithm consumes numeric state/input/parameter vectors.
...
- Simulate input/timestep/solver diagnostics use the checklist canonical codes:
  ...
  `E-SIM-TOLERANCE-INVALID`, and `E-SIM-SOLVER-UNSUPPORTED`;
  method-specific shape guards use `E-SIM-SYSTEM-SHAPE-UNSUPPORTED`.
- Error-message fixtures cover missing TimeSeries input, non-TimeSeries input,
  ...
  invalid tolerance, missing/unsupported solver, unsupported method-specific system shape
...
- Runtime residual evaluation accepts solver-provided tolerance and
  ...
  user-facing `residual_scale`/`residual_scales` options while broader component
  paths still default to unit/quantity scale policy.
