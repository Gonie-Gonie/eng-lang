...
- Solver algorithm seeds: dense linear solve with finite matrix/RHS/tolerance
  ...
  materialization, tolerance, max-iteration, timestep diagnostics, and IDE
  TimeSeries/solver-inspector rows from that internal `SolverResult` adapter.
  Newton/DAE/dynamic component seeds are not wired into language-level nonlinear
