...
- DateTime-indexed table metadata, row-level CSV runtime pages, source hash
  ...
  selection, `typed_payload.table_transforms[]` records for filter/select/sort/require_one/join
  row counts, selected columns, sort keys, predicate evidence, and join key pair counts,
...
- General table derive formulas and arbitrary TimeSeries expression execution.
