...
|---|---|---|---|
| Uncertainty / distribution numeric | `Internal` | `Internal` | scalar runtime numeric payloads now distinguish Certain/uncertain representations; narrow arithmetic propagation and validated `with { uncertainty = ... }` policy metadata are reviewable; keep internal until TimeSeries uncertainty, explicit validation semantics, IDE projection, and tests align |
