...
- State-space metadata and runtime seeds: typed vector/operator declarations,
  ...
  non-rectangular matrix diagnostics, unitful matrix-entry diagnostics,
...
- Solver algorithm seeds: dense linear solve, fixed-point iteration with
  ...
  contract plus failure diagnostics. Component solver result artifacts can carry
  state/algebraic trajectory summaries and points from that internal
  `SolverResult` adapter. Newton/DAE/dynamic component seeds are not
...
- Runtime optimization/JIT/AOT track: `eng_jit`, `eng.exe jit-plan`,
  ...
  residual, state-space RHS, finite-difference Jacobian, and Newton-step
  kernels, per-candidate executor/fallback reasons, and no native speedup
  claim.
