...
| LSP/editor check | `dev.bat lsp-check` | Runs library, persistent-stdio, semantic-token, visual-contract, exact-source, token-free-trivia, variable-width scalar-binding, backward-alias, multi-line rename, position-shifting trivia suffix rechecks, full-fallback, and recursive import-invalidation regressions. |
