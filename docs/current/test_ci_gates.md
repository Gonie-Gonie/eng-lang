...
| LSP/editor check | `dev.bat lsp-check` | Runs library, persistent-stdio, semantic-token, visual-contract, exact-source, token-free-trivia, variable-width numeric-binding suffix recheck, full-fallback, and recursive import-invalidation regressions. |
