...
| LSP/editor check | `dev.bat lsp-check` | Runs library, persistent-stdio, semantic-token, visual-contract, exact-source, token-free-trivia, variable-width scalar-binding and backward-alias suffix rechecks, full-fallback, and recursive import-invalidation regressions. |
