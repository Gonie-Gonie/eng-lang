...
| LSP/editor check | `dev.bat lsp-check` | Runs library, persistent-stdio, semantic-token, visual-contract, exact-source, token-free-trivia, variable-width scalar-binding, backward-alias and pure scalar-arithmetic type propagation, homogeneous explicit-scalar annotation/RHS/cardinality rechecks, rename/cardinality changes, resized/inserted/removed trivia and line-ending suffix rechecks, strict full-fallback, and recursive import-invalidation regressions. |
