...
| Native IDE check | `dev.bat ide-check` | Checks the native app contract, compiler-backed call help, nested/comment/declaration context filtering, UTF-16 caret forwarding, dirty-import snapshots, exact report reuse, bounded trivia/scalar-suffix incremental rechecks, strict context/TTL full fallback, overload rendering, stale-result rejection, Rust compilation, IDE smoke, and the shared VS Code extension contract. |
