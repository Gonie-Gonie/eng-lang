...
Remaining gap: native `eng.net`, `eng.cache`,
`eng.sampling`, `eng.case`, `eng.db`, and
`eng.model` module syntax remains planned. Fixture evidence must stay
labeled as composite workflow foundations, not full native module support.
