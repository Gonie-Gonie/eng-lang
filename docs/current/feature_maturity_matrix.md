...
| Side-effect/general programming policy | Preview | v0.4 implements typed path defaults/helpers, provenance-visible `exists`, and read-only UTF-8 `read text/json/toml` expressions with source hashes | Structured JSON/TOML values, broad write I/O, filesystem mutation, process execution, and network are not public-supported | Add write/export hardening and output manifest |
