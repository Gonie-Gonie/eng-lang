...
| JIT kernel planning | Experimental | `eng_jit`, `eng.exe jit-plan`, native IDE Runtime Summary display, and `eng-kernel-plan-v1` hot-kernel metadata for TimeSeries arithmetic/statistics/integration and system residual interface seeds | No native code generation or speedup claim | v1.4 gate |
