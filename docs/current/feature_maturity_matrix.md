...
| Side-effect/general programming policy | Preview | v0.3 implements typed path defaults/helpers and provenance-visible `exists`; docs retain typed file/process/network policy and profile intent for later phases | Broad runtime read/write I/O, filesystem mutation, process execution, and network are not public-supported | Add read-only text/json/toml policy seed |
