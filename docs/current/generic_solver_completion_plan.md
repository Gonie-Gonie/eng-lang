...
- 2026-06-21: Dynamic component derivative Newton fallback now consumes `with { residual_scale = ... }` and `with { residual_scales = [...] }` overrides for named source residuals. The runtime fixture `tests/runtime/dynamic_component_derivative_residual_scale_override.eng` proves derivative Newton diagnostics preserve raw residual values separately from normalized values, and diagnostics fixtures prove invalid, affine-derivative, and algebraic dynamic residual scale requests fail explicitly with `E-SOURCE-RESIDUAL-SCALE`. This broadens W4 residual scaling evidence into selected dynamic component derivative Newton solving without claiming broad algebraic dynamic-component scale policy, broad component nonlinear solving, or a complete scale-policy language.
