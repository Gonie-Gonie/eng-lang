...
| Component parameters | System-local component instances are zero-argument templates. | Constructor arguments and parameter binding must be typed, unit-checked, serialized in artifacts, and available to residual assembly. |
...
| 3. Parameterized components | Support typed constructor arguments, defaults, unit conversion, artifact serialization, and diagnostics for bad arity/type/unit. | Official parameterized component example plus diagnostics for missing/extra/incompatible arguments. |
