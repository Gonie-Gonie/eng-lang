...
- 2026-06-22: Static algebraic source systems now lower directly into the residual-graph solve path through `solve <SystemName>`. Square systems without `der(...)` equations support `dense_linear`/`linear` for linear residuals, `fixed_point` for direct or affine single-target `eq` side mappings, and `newton`/`nonlinear_newton` for nonlinear residuals, treating source `state`/`output` variables as static algebraic unknowns and parameter/scalar input defaults as residual constants. Runtime unit tests, CLI smoke fixtures `tests/runtime/source_system_dense_linear_solve.eng`, `tests/runtime/source_system_fixed_point_solve.eng`, `tests/runtime/source_system_fixed_point_affine_solve.eng`, and `tests/runtime/source_system_newton_solve.eng`, `tests/runtime/source_system_newton_source_linear_jacobian.eng`, plus `tests/runtime/source_system_newton_variable_scales.eng`, while `tests/diagnostics/source_system_newton_nonconvergence.eng` proves explicit nonconvergence failure artifacts; report-spec JSON and report HTML prove solved variables, residuals, iteration diagnostics, source lines, Jacobian policy, variable-scale policy, convergence status, and failure codes. This advances generic equation-system work while leaving dynamic, DAE, event, component-coupled, and broad unit-expression solving in the planned lanes.
