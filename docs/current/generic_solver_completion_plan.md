...
- 2026-06-22: Source `solve component_graph` fixed-point updates now fall back from pivotable linear residual terms to parsed source-expression mappings when an equation side is a single unknown, such as `loop.source.x eq cos(loop.target.x)`. The bridge evaluates final residual artifacts through the parsed source residual AST, keeps user-facing residual tolerance aligned with a stricter internal update tolerance, and adds runtime/CLI fixture coverage in `tests/runtime/fixed_point_expression_mapping.eng` for solved variables, residual expressions, convergence status, report-spec JSON, and report HTML. This broadens fixed-point evidence beyond linear pivots without claiming arbitrary partitioning or a general fixed-point extraction policy.

- 2026-06-22: Static algebraic source systems now lower directly into the residual-graph solve path through `solve <SystemName>`. Square systems without `der(...)` equations support `dense_linear`/`linear` for linear residuals, `fixed_point` for direct single-unknown `eq` side mappings, and `newton`/`nonlinear_newton` for nonlinear residuals, treating source `state`/`output` variables as static algebraic unknowns and parameter/scalar input defaults as residual constants. Runtime unit tests, CLI smoke fixtures `tests/runtime/source_system_dense_linear_solve.eng`, `tests/runtime/source_system_fixed_point_solve.eng`, and `tests/runtime/source_system_newton_solve.eng`, report-spec JSON, and report HTML prove solved variables, residuals, iteration diagnostics, source lines, and convergence status. This advances generic equation-system work while leaving dynamic, DAE, event, component-coupled, and broad unit-expression solving in the planned lanes.
...
| Fixed-point source solve is no longer pivotable-linear only, but expression mapping is limited to direct single-unknown `eq` sides. | Detect general `x = g(x)` partitions from shared residual IR, add variable scaling, relaxation policy, residual history, and nonconvergence artifacts. | Success/failure fixtures cover coupled fixed-point loops, bad partitions, non-finite updates, tolerance/max-iteration options, and report/IDE residual history. Current expression-mapping evidence includes `tests/runtime/fixed_point_expression_mapping.eng` and static source-system coverage in `tests/runtime/source_system_fixed_point_solve.eng`. | Move fixed-point update extraction from residual-shape helper to shared residual IR metadata. |
