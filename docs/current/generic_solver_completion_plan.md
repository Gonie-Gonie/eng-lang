...
- 2026-06-22: Static algebraic source systems now lower directly into the residual-graph solve path through `solve <SystemName>`. Square systems without `der(...)` equations support `dense_linear`/`linear` for linear residuals and `newton`/`nonlinear_newton` for nonlinear residuals, treating source `state`/`output` variables as static algebraic unknowns and parameter/scalar input defaults as residual constants. Runtime unit tests, CLI smoke fixtures `tests/runtime/source_system_dense_linear_solve.eng` and `tests/runtime/source_system_newton_solve.eng`, report-spec JSON, and report HTML prove solved variables, residuals, iteration diagnostics, source lines, and convergence status. This advances generic equation-system work while leaving dynamic, DAE, event, component-coupled, and broad unit-expression solving in the planned lanes.
