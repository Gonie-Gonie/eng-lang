...
| Component parameters | System-local component instances support empty constructors plus declared component parameter defaults plus named or declaration-order positional constructor overrides for numeric literal, importable const, or pure arithmetic expression values, with unit checks, diagnostics, review/report-spec parameter provenance, and qualified parameter references preserved into component residual dependencies. Static linear residual assembly plus source Newton/DAE residual evaluation now consume materialized parameter values, including the official parameterized nonlinear HeatRate fixture. | Remaining work is canonical solver-unit policy across all source paths, args/object/non-arithmetic constructor bindings, and full expression lowering for broader parameterized nonlinear/dynamic terms. |
| Component equations | Linear port-signal equations, literal boundary seeds, compiler diagnostics for incompatible unitful constants, and shared runtime arithmetic linearization for parenthesized linear residuals, division by constants, unit literals, and parameter constants cover the public component solve path. | Component-local equations must support unit-parameterized expressions, nonlinear terms, derivative terms, and robust dependency extraction. |
| Algebraic nonlinear solve | Newton is bridged for narrow coupled multi-variable source residuals, including unitful HeatRate residuals with a numeric component parameter. | Multi-variable nonlinear residual graphs with scaling, Jacobian policy, line search or trust region, diagnostics, and failure artifacts. |
...
| DAE solve | Multi-state unitful temperature implicit-Euler DAE bridge with identity/mass-matrix fallback and vector initial values. | State/algebraic partitioning, consistent initialization, mass matrix support, events, adaptive stepping, and higher-order methods. |
