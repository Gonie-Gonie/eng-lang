...
| Component parameters | System-local component instances support empty constructors plus declared component parameter defaults and named constructor overrides for numeric literal values, with unit checks, diagnostics, review/report-spec parameter provenance, and qualified parameter references preserved into component residual dependencies. Static linear residual assembly and source residual evaluation now consume materialized parameter values. | Remaining work is broad constructor arity/default policy, canonical solver-unit policy across all source paths, non-literal bindings, and full expression lowering for parameterized nonlinear/dynamic terms. |
...
| Algebraic nonlinear solve | Newton is bridged for narrow coupled multi-variable source residuals, including unitful HeatRate residuals. | Multi-variable nonlinear residual graphs with scaling, Jacobian policy, line search or trust region, diagnostics, and failure artifacts. |
