...
- 2026-06-21: Algebraic-free `dynamic_component_adaptive_heun` source solves now reuse adaptive Heun over parsed derivative residual expressions and emit fixed output-grid component trajectories plus accepted-substep diagnostics in runtime/report artifacts. A diagnostics fixture keeps algebraic dynamic component graphs explicit as `E-DYNAMIC-COMPONENT-ADAPTIVE-ALGEBRAIC`, so this does not claim broad adaptive component timestepping, nonlinear algebraic coupling, behavior coupling, or event support.
