...
| `eng.stats` | Native workflow support | Compiler/runtime | `typed_payload.statistics`<br>`report_spec.computed_statistics`<br>`review_document.calculations`<br>`review_document.report_outputs` | `W-STATS-SUM-001` | `examples/official/01_csv_plot`<br>`examples/workflows/03_uncertain_sensor_report` | `cargo test -p eng_compiler stats`<br>`cargo test -p eng_runtime computes_heat_rate_statistics_and_integral` |
