...
| `eng.table` | Native workflow support | Compiler/runtime | `review.inputs`<br>`review_document.table_transforms`<br>`typed_payload.table_diagnostics`<br>`typed_payload.table_selections`<br>`typed_payload.table_transforms` | `E-TABLE-UNKNOWN-COLUMN`<br>`E-TABLE-PREDICATE-TYPE`<br>`E-TABLE-JOIN-KEY-MISMATCH`<br>`E-TABLE-SCHEMA-MISMATCH` | `examples/workflows/01_weather_api_to_standard_file`<br>`examples/workflows/02_native_surrogate_case_workflow` | `cargo test -p eng_runtime table_`<br>`cargo test -p eng_compiler table_` |
...
| `eng.workflow` | Native workflow support | Compiler/runtime | `run_plan`<br>`run_lock`<br>`output_manifest`<br>`run_log` | `run_lock_changed`<br>`artifact_hash_mismatch` | `examples/workflows/01_weather_api_to_standard_file`<br>`examples/workflows/02_native_surrogate_case_workflow` | `cargo test -p eng_runtime run_plan` |
