...
`typed_payload.timeseries_fill[]` records for `fill missing` interpolation or
explicit `record_only` policies, downstream native statistics and unit-correct
HeatRate integration for materialized fill outputs, `typed_payload.timeseries_quality[]` summaries that combine coverage
...
native alignment/resampling outputs in
...
|---|---|---|---|---|---|---|
| `eng.path` | Supported | Compiler/runtime | `review.inputs`<br>`review.environment_dependencies` | `E-PATH-INVALID`<br>`E-PATH-TRAVERSAL`<br>`E-PATH-OUTSIDE-OUTPUT-ROOT`<br>`E-ENV-CALL-001`<br>`E-ENV-MISSING-001` | `examples/official/10_path_policy` | `cargo test -p eng_compiler path_policy`<br>`cargo test -p eng_compiler value_constructor`<br>`cargo test -p eng_runtime value_constructor` |
...
| `eng.table` | Native workflow support | Compiler/runtime | `review.inputs`<br>`review_document.table_transforms`<br>`typed_payload.temporal_values`<br>`object_store.Date`<br>`typed_payload.table_diagnostics`<br>`typed_payload.table_selections`<br>`typed_payload.table_transforms` | `E-DATE-CALL-001`<br>`E-DATE-ARG-001`<br>`E-DATE-VALUE-001`<br>`E-DATE-RESULT-001`<br>`E-TABLE-UNKNOWN-COLUMN`<br>`E-TABLE-PREDICATE-TYPE`<br>`E-TABLE-JOIN-KEY-MISMATCH`<br>`E-TABLE-SCHEMA-MISMATCH`<br>`W-TABLE-LEGACY-SELECT-FIRST-ROW` | `examples/workflows/01_weather_api_to_standard_file`<br>`examples/workflows/02_native_surrogate_case_workflow` | `cargo test -p eng_compiler native_date`<br>`cargo test -p eng_runtime native_date`<br>`cargo test -p eng_lsp native_date`<br>`cargo test -p eng_runtime table_`<br>`cargo test -p eng_compiler table_` |
| `eng.timeseries` | Native workflow support | Compiler/runtime | `typed_payload.numeric_values`<br>`typed_payload.timeseries_coverage`<br>`typed_payload.timeseries_fill`<br>`typed_payload.timeseries_quality`<br>`typed_payload.quality_results`<br>`typed_payload.time_alignments`<br>`typed_payload.statistics`<br>`typed_payload.integrations`<br>`object_store.TimeSeries`<br>`report_spec.time_alignments`<br>`report_spec.computed_statistics`<br>`report_spec.computed_integrations`<br>`review.fallbacks` | `E-TIMESERIES-COVERAGE-GAP`<br>`E-TIMESERIES-FILL-BINDING`<br>`E-TIMESERIES-FILL-METHOD`<br>`E-TIMESERIES-FILL-STEP`<br>`E-TIMESERIES-FILL-STEP-CONFLICT`<br>`E-TIMESERIES-FILL-MAX-GAP`<br>`W-TIMESERIES-FILL-METHOD-IMPLICIT`<br>`E-TIMESERIES-ALIGN-BINDING`<br>`E-TIMESERIES-ALIGN-METHOD`<br>`E-TIMESERIES-ALIGN-STEP`<br>`E-TIMESERIES-ALIGN-TOLERANCE`<br>`W-TIMESERIES-RESAMPLE-STEP-REDUNDANT`<br>`W-FALLBACK-USED` | `examples/official/01_csv_plot`<br>`examples/internal/17_measured_vs_simulated`<br>`examples/workflows/01_weather_api_to_standard_file`<br>`examples/workflows/03_uncertain_sensor_report` | `cargo test -p eng_compiler timeseries_fill_missing`<br>`cargo test -p eng_compiler timeseries_alignment`<br>`cargo test -p eng_runtime computes_heat_rate_statistics_and_integral`<br>`cargo test -p eng_runtime run_file_materializes_explicit_computed_scalars_in_declared_units`<br>`cargo test -p eng_runtime run_file_records_timeseries_coverage_in_review`<br>`cargo test -p eng_runtime run_file_records_timeseries_fill_missing_in_artifacts`<br>`cargo test -p eng_runtime run_file_materializes_timeseries_alignment_and_resampling_outputs` |
...
| `eng.quality` | Native workflow support | Compiler/runtime | `typed_payload.numeric_values`<br>`typed_payload.metrics`<br>`typed_payload.expectation_suites`<br>`typed_payload.quality_results`<br>`typed_payload.validations`<br>`typed_payload.policy_results`<br>`review.expectation_suites`<br>`review.quality_results`<br>`review.validations`<br>`report_spec.computed_metrics`<br>`report_spec.quality_report`<br>`report_html.quality_report`<br>`ide.quality_inspector`<br>`output_manifest` | `E-RMSE-CALL-001`<br>`E-TABLE-SCHEMA-MISMATCH`<br>`W-FALLBACK-USED` | `examples/internal/17_measured_vs_simulated`<br>`examples/diagnostics/data_quality` | `cargo test -p eng_compiler rmse_call_and_command_forms_share_one_native_contract`<br>`cargo test -p eng_compiler explicit_computed_scalars_share_native_validation_and_metadata`<br>`cargo test -p eng_runtime materializes_call_style_rmse_with_native_unit_conversion`<br>`cargo test -p eng_runtime run_file_materializes_explicit_computed_scalars_in_declared_units`<br>`cargo test -p eng_runtime run_file_materializes_timeseries_alignment_and_resampling_outputs`<br>`cargo test -p eng_runtime run_file_records_common_quality_results_for_validation_and_schema_constraints` |
| `eng.template` | Native workflow support | Compiler/runtime | `typed_payload.render_manifests`<br>`template_render_manifest`<br>`review.render_manifests`<br>`output_manifest` | `E-RENDER-CALL-001`<br>`E-TEMPLATE-MISSING-VALUE` | `examples/workflows/02_native_surrogate_case_workflow` | `cargo test -p eng_compiler render_template_command_lowers_with_template_contract`<br>`cargo test -p eng_runtime template` |
...
| `eng.report` | Native workflow support | eng_report | `report_spec`<br>`report_html`<br>`review_document.runtime_result`<br>`output_manifest` | - | `examples/workflows/03_uncertain_sensor_report` | `cargo test -p eng_report report_html_`<br>`cargo test -p eng_runtime run_source_materializes_uncertainty_validation_results`<br>`dev.bat workflows-test` |
| `eng.stats` | Native workflow support | Compiler/runtime | `typed_payload.numeric_values`<br>`typed_payload.statistics`<br>`report_spec.computed_statistics`<br>`review_document.calculations`<br>`review_document.report_outputs` | `E-STATS-DURATION-CALL-001`<br>`E-STATS-DURATION-SOURCE-001`<br>`E-STATS-DURATION-UNIT-001`<br>`W-STATS-SUM-001` | `examples/official/01_csv_plot`<br>`examples/workflows/03_uncertain_sensor_report` | `cargo test -p eng_compiler duration_above_`<br>`cargo test -p eng_compiler explicit_computed_scalars_share_native_validation_and_metadata`<br>`cargo test -p eng_runtime computes_heat_rate_statistics_and_integral`<br>`cargo test -p eng_runtime run_file_materializes_explicit_computed_scalars_in_declared_units`<br>`cargo test -p eng_jit executes_native_sum_statistic_reduction` |
...
evidence for `eng.timeseries`, `eng.stats`, `eng.uncertainty`, `eng.report`, and
`eng.plot`.
...
## Native Case Calculation

The supported native calculation form is:

```englang
case_runs = apply run_case over case_inputs
with {
    results = {
        annual_energy = base_energy + load_index * 500 kWh
        unmet_hours = base_hours - cooling_cop * 0.5 h
    }
...
`results` is required and every entry is evaluated by EngLang's typed numeric
