...
| `eng.io` | Supported | Compiler/runtime | `review.inputs`<br>`review.side_effects`<br>`output_manifest` | `E-IO-JSON-PARSE`<br>`E-IO-TOML-PARSE`<br>`E-IO-JSON-FIELD-ACCESS-001` | `examples/official/11_read_only_io` | `cargo test -p eng_compiler read_only_io` |
| `eng.fs` | Supported narrow | Compiler/runtime | `review.side_effects`<br>`output_manifest`<br>`run_log` | `E-FS-CONFIRM-001`<br>`E-FS-DELETE-001`<br>`E-PROFILE-SAFE-FS` | `examples/official/13_file_operations` | `cargo test -p eng_compiler file_operations` |
| `eng.log` | Supported | Compiler/runtime | `run_log` | `none_current` | `examples/official/15_process_result` | `cargo test -p eng_runtime run_file` |
...
| `eng.artifact` | Native workflow support | Compiler/runtime | `output_manifest`<br>`review.side_effects`<br>`artifact_registry` | `artifact_validation_failed` | `examples/workflows/01_weather_api_to_standard_file`<br>`examples/workflows/02_external_simulation_surrogate` | `cargo test -p eng_runtime output_manifest` |
| `eng.review` | Native workflow support | Compiler/runtime | `review` | `semantic_diff_changed`<br>`review_risk` | `eng review examples/workflows/01_weather_api_to_standard_file/main.eng` | `cargo test -p eng_compiler review_json_exposes_normalized_review_document`<br>`cargo test -p eng_cli review_semantic_diff_compares_workflow_modules` |
...
| `eng.workflow` | Native workflow support | Compiler/runtime | `run_plan`<br>`run_lock`<br>`output_manifest`<br>`run_log` | `run_lock_changed`<br>`artifact_hash_mismatch` | `examples/workflows/01_weather_api_to_standard_file`<br>`examples/workflows/02_external_simulation_surrogate` | `cargo test -p eng_runtime run_plan` |
| `eng.report` | Planned | No executable backing | `report`<br>`review`<br>`output_manifest` | `none_current` | `examples/workflows/03_uncertain_sensor_report` | `cargo test -p eng_report` |
...
| `eng.plot` | Planned | No executable backing | `plotspec`<br>`plot_manifest`<br>`output_manifest` | `none_current` | `examples/official/01_csv_plot` | `cargo test -p eng_report plot` |
